...
이 문서는 2024년 2차 리팩토링에서 추가‧변경된 내용을 처음 접하는 개발자도 쉽게 이해할 수 있도록 정리한 기록이다. 목표는 `rebuild_spans()`에 의존하던 이전 코드에서 탈피해 **증분(span) 갱신**을 일관되게 유지하면서, 실행 중에 스팬을 재계산하지 않아도 되는 구조로 옮겨가는 것이었다.
